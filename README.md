...
> 
> [598. Range Addition II (有点弱智的一道题，找最小的重叠的部分)]
> 
> [492. Construct the Rectangle (开方。。。)]
> 
> [455. Assign Cookies (分配cookie，输出最多能满足多少个小孩，greed和size都排序，然后都从后开始遍历，如果这个小孩不满意当前的饼干，那就直接给下一个小孩，因为没有更大的饼干能满足他了；如果给他已经分配给其他人的更大的饼干，满足的小孩的总数也没变，所以就不给他了。)]
> 
...
> 
> [35. Search Insert Position (还是二分查找)]
> 
> [263. Ugly Number (只包含2，3，5）的因数]
> 
> [645. Set Mismatch (给一个数组包含从1-n，然后有一个数字是重复的出现了两次，有一个数字丢失了，然后要的是找到重复的数字和丢失的数字。用异或就行了，先1-n异或，然后用一个字典保存第一次见到的数字，然后异或)]
> 
